...
function Nullwire:Notify(config) if self._activeWindow then return self._activeWindow:Notify(config) end end
function Nullwire:CreateWindow(options) return Nullwire.new(options) end

return Nullwire

-- Distribution contract: fetch this file, evaluate it with loadstring, and use the returned Library object.
